...
imported_data <- read.csv("/Users/hryu/Downloads/sensitivity_results_flattened-2.csv", row.names = 1, check.names = FALSE)


data <- imported_data[-c(7,9,10,20,24,28,50,53,70,71,72,85:88,99:100,109:110,123:125,134,147:150,165:197), ]
...
top_20_indices <- ranked_parameters[1:20]  # Get the first 20 most sensitive ones after exclusion
...
math_labels <- c(
  ...
  expression(d[N]),
  expression(delta[I*","*T])
)



new_order <- c(2, 6, 7, 10, 17, 11:13, 3, 18, 4,8,16,19,1,9,5,20, 14:15)
data_top_20 <- data_top_20[new_order, , drop = FALSE]
math_labels <- math_labels[new_order]

group_labels <- c("IFN I", "G-CSF", "IL-6", "Immune cells", "Lung cells", "Virus")
group_sizes  <- c(2, 3, 3, 6, 4, 2)  # must sum to 20
...
# Set output path
heatmap_file <- file.path(output_dir, "top20_heatmap.pdf")

# Open PDF device
pdf(file = heatmap_file, width = 8.5, height =9)
...
ht <- Heatmap(
  ...
  row_names_gp = gpar(fontsize = 14),  # ← enlarge font
